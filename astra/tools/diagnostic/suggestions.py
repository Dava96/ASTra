...
ERROR_PATTERNS = [
    ...
    (re.compile(r"TypeError: (.+) takes (\d+) positional arguments? but (\d+) (?:was|were) given", re.IGNORECASE),
     "Function called with wrong number of arguments ({2} expected, {3} given)"),

    (re.compile(r"AttributeError: ['\"]?(\S+)['\"]? object has no attribute ['\"]?(\S+)['\"]?", re.IGNORECASE),
