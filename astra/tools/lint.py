...
LINTER_CONFIG = {
    "python": {
        "linters": [
            {"name": "ruff", "check_cmd": ["ruff", "check", "."], "fix_cmd": ["ruff", "check", "--fix", "."], "type_check": False},
    ...
    "javascript": {
        "linters": [
            {"name": "eslint", "check_cmd": ["npx", "eslint", "."], "fix_cmd": ["npx", "eslint", "--fix", "."], "type_check": False}
        ],
    ...
    "php": {
        "linters": [
            {"name": "phpstan", "check_cmd": ["vendor/bin/phpstan", "analyse"], "fix_cmd": None, "type_check": True}
        ],
        "detect_files": ["*.php", "composer.json"]
    },
    "go": {
        "linters": [
            {"name": "go-vet", "check_cmd": ["go", "vet", "./..."], "fix_cmd": None, "type_check": False},
            {"name": "gofmt", "check_cmd": ["gofmt", "-l", "."], "fix_cmd": ["gofmt", "-w", "."], "type_check": False}
        ],
        "detect_files": ["*.go", "go.mod"]
    },
    "rust": {
        "linters": [
            {"name": "clippy", "check_cmd": ["cargo", "clippy"], "fix_cmd": ["cargo", "clippy", "--fix", "--allow-dirty"], "type_check": True}
        ],
