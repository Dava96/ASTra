...
async def start_discord_bot():
    ...
    orchestrator = Orchestrator(gateway, config)
    queue = TaskQueue()
...
def ingest(
    ...
    ast_depth: int | None = typer.Option(None, "--ast-depth", help="AST parsing granularity (1=Signatures, 3=Full)")
):
...
def estimate_size(
    ...
    print(f"📊 Estimating size for {path} (Depth: {ast_depth}, Sample: {int(sample_rate*100)}%)...")
