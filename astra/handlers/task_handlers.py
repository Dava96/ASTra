...
class TaskHandlers:
    ...
    def __init__(self, gateway: Gateway, orchestrator: Orchestrator, queue: TaskQueue, config: Config):
    ...
    async def handle_feature(self, cmd: Command):
        ...
        if not request:
            await self.gateway.send_followup(cmd.raw_interaction, "❌ Please provide a feature description")
            return

        project = self.orchestrator.get_active_project()
        if not project:
            await self.gateway.send_followup(cmd.raw_interaction, "❌ No active project. Please use `/checkout` first.")
            return
    ...
    async def handle_fix(self, cmd: Command):
        ...
        request = cmd.args.get("request", "")
        if not request:
            await self.gateway.send_followup(cmd.raw_interaction, "❌ Please provide a bug description")
            return
        ...
        await self.gateway.send_followup(
            cmd.raw_interaction,
            f"🐛 Bug fix queued (ID: `{task.id}`, Position: {position})"
        )
    ...
    async def handle_quick(self, cmd: Command):
        ...
        if not file_path or not change:
            await self.gateway.send_followup(cmd.raw_interaction, "❌ Usage: /quick <file> <change>")
            return
        ...
        await self.gateway.send_followup(cmd.raw_interaction, f"⚡ Quick edit queued (ID: `{task.id}`)")
    ...
    async def handle_history(self, cmd: Command):
        ...
        if not status['recent']:
            await self.gateway.send_followup(cmd.raw_interaction, "ℹ️ No task history")
            return

        msg = "📜 **Task History**\n\n"
        for t in status['recent'][-10:]:
            emoji = "✅" if t['status'] == 'success' else "❌"
    ...
    async def handle_approve(self, cmd: Command):
        ...
        try:
            ...
            await self.gateway.send_followup(cmd.raw_interaction, f"✅ Task `{task_id}` approved and queued for execution.")
    ...
    async def handle_revise(self, cmd: Command):
        ...
        try:
            ...
            await self.gateway.send_followup(cmd.raw_interaction, f"📝 Task `{task_id}` queued for revision.")
        except Exception as e:
