...
class ProjectHandlers:
    ...
    def __init__(self, gateway: Gateway, orchestrator: Orchestrator, config: Config, clock: Clock = None):
    ...
    async def handle_checkout(self, cmd: Command):
        ...
        if not repo_url:
            await self.gateway.send_followup(cmd.raw_interaction, "❌ Please provide a repository URL")
            return
        ...
        if repo_dir.exists() and (repo_dir / ".git").exists():
            await self.gateway.send_followup(cmd.raw_interaction, f"📂 Project `{repo_name}` already exists. Switching...")
        else:
            await self.gateway.send_followup(cmd.raw_interaction, f"🛰️ Cloning {repo_url}...")
            result = await vcs.clone(repo_url, dest)
            if not result.success:
                await self.gateway.send_followup(cmd.raw_interaction, f"❌ Clone failed: {result.error}")
                return
        ...
        try:
            ...
            async def on_progress(percent, current, total):
                 ...
                 if current_time - last_update_time > UPDATE_INTERVAL_SECONDS:
                      if channel_id and hasattr(self.gateway, 'send_progress'):
            ...
            if channel_id:
                try:
                    from astra.interfaces.gateway import Message
                    await self.gateway.send_message(Message(
                        content=f"⚠️ Error indexing `{repo_name}`: {e}",
