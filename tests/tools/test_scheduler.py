...
from astra.config import Config, SchedulerConfig, get_config
...
async def test_cron_tool_run_now(scheduler_db):
    ...
    res = await tool.execute("schedule", cron="* * * * *", command="echo manual_run", project_path="/tmp")
    import re
    ...
    with patch("astra.tools.scheduler.service.execute_job_wrapper", new_callable=AsyncMock) as mock_wrapper:
...
async def test_resource_guard_skips_execution():
    ...
    with patch("astra.tools.scheduler.service.psutil") as mock_psutil:
        ...
        res = await execute_job_wrapper("echo skip", "/tmp", resource_guard=True, max_memory_percent=90)
...
async def test_job_cancellation(scheduler_db):
    ...
    tool = CronTool()
    await tool.execute("schedule", cron="* * * * *", command="echo cancel_me", project_path="/tmp/cancel")
...
async def test_run_job_now_errors(scheduler_db):
    ...
    with patch("astra.tools.scheduler.service.execute_job_wrapper", side_effect=Exception("Wrapper failed")):
