...
from astra.tools.memory.tool import MemoryTool, _RECALL_CACHE
from astra.tools.memory.models import MemoryOperationResult
...
async def test_memory_lifecycle_structured(memory_tool):
    ...
    with patch.object(memory_tool.store, 'query', wraps=memory_tool.store.query) as mock_query:
        res_cached = await memory_tool.execute("recall", query="blue", project_name=project, format="dict")
    ...
    with patch.object(memory_tool.store, 'query', wraps=memory_tool.store.query) as mock_query_2:
        res_new = await memory_tool.execute("recall", query="blue", project_name=project, format="dict")
        assert res_new["data"][0]["content"] == "User likes dark blue."
        mock_query_2.assert_called()

    # 5. Forget
    res = await memory_tool.execute("forget", memory_id=memory_id, project_name=project, format="dict")
    assert res["success"] is True
...
async def test_deduplication(memory_tool):
    ...
    # First Add
    res1 = await memory_tool.execute("remember", content=content, project_name=project, format="dict")
    assert res1["success"] is True
    id1 = res1["data"][0]["id"]

    # Second Add (Duplicate)
    res2 = await memory_tool.execute("remember", content=content, project_name=project, format="dict")
...
async def test_markdown_format(memory_tool):
    ...
    project = "md_proj"
    res = await memory_tool.execute("remember", content="Text", project_name=project, format="markdown")
