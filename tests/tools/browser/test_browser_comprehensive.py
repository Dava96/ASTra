...
def browser_tool():
    with patch("astra.tools.browser.tool.get_config") as mock_config:
        def mock_get(*args, **kwargs):
            if "cleanup_after_hours" in args: return 1
            if "screenshot_dir" in args: return "./data/screenshots"
...
def get_async_page_mock():
    ...
    pm.accessibility.snapshot = AsyncMock(return_value={"role":"root","name":"n","children":[]})
...
def test_registry_and_base():
    class TestNav(BaseNavigator):
        name = "test_nav"
        async def find_element(self, selector, **kwargs): return "el"
        async def perform_action(self, action, selector, **kwargs): return NavigationResult(success=True)
    ...
    class BadNav(BaseNavigator):
        name = ""
        async def find_element(self, s, **kw): pass
        async def perform_action(self, a, s, **kw): pass
    with pytest.raises(ValueError): register_navigator(BadNav)
    tn = TestNav(); tn.set_page("p"); assert tn.page == "p"
...
async def test_lifecycle_context(browser_tool):
    with patch("astra.tools.browser.tool.async_playwright") as mock_pw_func:
        mock_pw = AsyncMock(); mock_pw_func.return_value = mock_pw; mock_pw.start.return_value = mock_pw
...
async def test_tool_dispatch_100(browser_tool):
    ...
    a11y = A11yNode(role="button", name="btn"); a11y.children = [A11yNode(role="text", name="txt")]
...
async def test_interact_logic_exhaustive(browser_tool):
    ...
    n_ok = MagicMock(); n_ok.perform_action = AsyncMock(return_value=NavigationResult(success=True, data={"v":1}))
    ...
    # Hybrid fallback
    n_a = MagicMock(); n_a.perform_action = AsyncMock(return_value=NavigationResult(success=False))
    n_d = MagicMock(); n_d.perform_action = AsyncMock(return_value=NavigationResult(success=True))
    with patch("astra.tools.browser.tool.get_navigator", side_effect=lambda n: n_a if n=="a11y" else n_d):
...
async def test_nav_impls_pure_100():
    ...
    # A11y
    a = A11yNavigator(); a.page = MagicMock()
    # Find regex hit
    ...
    with patch.object(a, "find_element", return_value=locR.first):
        await a.perform_action("click", "r:s"); await a.perform_action("type", "r:s", text="h")
        await a.perform_action("get_text", "r:s")
        assert (await a.perform_action("un", "r:s")).success is False
        locR.first.click.side_effect = Exception("e"); await a.perform_action("click", "r:s")
    # Not found / Exception
    with patch.object(a, "find_element", return_value=None):
        assert (await a.perform_action("click", "r:s")).success is False
    a.page.get_by_role.side_effect = Exception("f"); assert await a.find_element("r:n") is None

    # DOM
    d = DOMNavigator(); d.page = MagicMock()
    locD = MockLocator([1]); d.page.locator.side_effect = lambda s: locD
    assert await d.find_element("#i") is locD.first
    d.page.locator.side_effect = Exception("f"); assert await d.find_element("#i") is None
    with patch.object(d, "find_element", return_value=locD.first):
        await d.perform_action("click", "#i"); await d.perform_action("type", "#i", text="h")
        await d.perform_action("get_text", "#i")
        assert (await d.perform_action("un", "#i")).success is False
        locD.first.click.side_effect = Exception("e"); await d.perform_action("click", "#i")

def test_cleanup_and_compare_100(browser_tool, tmp_path):
    browser_tool._screenshot_dir = tmp_path
    f1 = tmp_path / "screenshot_old.png"; f1.write_text("o"); os.utime(str(f1), (0, 0))
    with patch("astra.tools.browser.tool.Path.unlink", side_effect=[Exception("f"), None]):
        browser_tool.cleanup(); browser_tool.cleanup()

    with patch("PIL.Image.open") as m_open:
        img = MagicMock(); img.size=(1,1); img.convert.return_value=img; m_open.return_value=img
        ...
        m_open.side_effect = Exception("err"); assert "err" in browser_tool.compare_screenshots("p1", "p2").summary
...
def test_wrappers_100():
    with patch.object(browser_tool_mod, "PLAYWRIGHT_AVAILABLE", False):
        with pytest.raises(ImportError): asyncio.run(BrowserTool().start())
    with patch.object(browser_tool_mod, "PILLOW_AVAILABLE", False):
    ...
    with patch("astra.tools.browser.tool.BrowserTool") as MT:
        mi = AsyncMock(); MT.return_value = mi; mi.__aenter__.return_value = mi
