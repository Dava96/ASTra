...
def test_tool_registry_compliance():
    ...
    for tool in tools:
        ...
        assert isinstance(tool.description, str), f"Tool {type(tool).__name__} description must be string"

        assert hasattr(tool, "parameters"), f"Tool {type(tool).__name__} missing 'parameters'"
        assert isinstance(tool.parameters, dict), f"Tool {type(tool).__name__} parameters must be dict"
        assert "type" in tool.parameters, f"Tool {type(tool).__name__} parameters missing 'type'"

        # Check if it implements Protocol
        assert isinstance(tool, Tool), f"Tool {type(tool).__name__} does not implement Tool protocol"
...
def test_all_tools_are_registered():
    ...
    tool_classes = []
    for loader, module_name, is_pkg in pkgutil.walk_packages(astra.tools.__path__, astra.tools.__name__ + "."):
        try:
            module = importlib.import_module(module_name)
            for name, obj in inspect.getmembers(module):
                if inspect.isclass(obj) and issubclass(obj, BaseTool) and obj is not BaseTool:
                    if obj.__module__ == module_name: # Ensure it's defined in THIS module, not imported
    ...
    gateway = MagicMock(spec=Gateway)
    orchestrator = Orchestrator(gateway)
    registered_names = [t.name for t in orchestrator._tools.list_tools()]
    ...
    for cls in tool_classes:
        ...
        if cls.__name__ in ("DiagnosticTool", "OutputParser", "PytestParser", "JestParser", "PhpunitParser", "BrowserConsoleParser"):
            continue
        ...
        assert found, f"Tool class {cls.__name__} (from {cls.__module__}) is not registered in Orchestrator"
