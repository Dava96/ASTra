...
class TestFullIntegration:
    ...
    async def test_ingestion_end_to_end(self, temp_env):
        ...
        # Patch config globally since it's imported locally in functions
        with patch("astra.config.get_config", return_value=mock_config):
            ...
            # 3. Run Ingestion
            # Note: We may need to wrap this if it attempts to load heavyweight models
            # but ChromaDBStore should handle the models if they are already downloaded
            ...
            assert graph._graph.has_edge(main_file_node, utils_file_node), f"Dependency edge {main_file_node} -> {utils_file_node} missing"

            # Verify AST node containment
            ast_nodes = [n for n, d in graph._graph.nodes(data=True) if d.get("file_path") == "main.py" and d.get("type") != "file"]
            assert len(ast_nodes) > 0
            for ast_node in ast_nodes:
                assert graph._graph.has_edge(ast_node, main_file_node), f"AST node {ast_node} should be linked to {main_file_node}"

            # Verify Ignore Patterns
            nodes = list(graph._graph.nodes)
            assert not any("ignored.py" in str(n) for n in nodes), "Node from .venv should be ignored"
            ...
            assert utils_file_node in deps, f"Graph should return {utils_file_node} as dependency of {main_file_node}"
