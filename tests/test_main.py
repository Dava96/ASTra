...
async def test_start_discord_bot():
    with patch('astra.adapters.gateways.discord.DiscordGateway') as mock_gateway, \
         patch('astra.core.orchestrator.Orchestrator') as mock_orch, \
         patch('astra.core.task_queue.TaskQueue') as mock_queue, \
         patch('astra.handlers.command_handlers.CommandHandler') as mock_handler, \
...
async def test_run_cli_task():
    ...
    with patch('astra.core.orchestrator.Orchestrator') as mock_orch, \
         patch('astra.core.task_queue.TaskQueue') as mock_queue:
