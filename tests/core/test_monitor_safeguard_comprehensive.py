...
def monitor():
    ...
    def mock_get(*args, **kwargs):
        if "threshold_gb" in args: return 10.0
...
def safeguard():
    ...
    def mock_get(*args, **kwargs):
        if "max_repo_size_mb" in args: return 500
        if "min_disk_space_mb" in args: return 1000
        if "max_memory_percent" in args: return 90.0
        return kwargs.get("default")
    cfg.get.side_effect = mock_get
...
def test_monitor_run_all(monitor):
    with patch.object(monitor, "check_disk_usage", return_value=(True, "ok")), \
