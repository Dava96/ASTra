...
class TestOrchestratorMocked:
    ...
    @pytest.fixture
    def orchestrator_mocks(self, mock_gateway, mock_config):
    ...
    def test_set_active_project(self, orchestrator_mocks):
    ...
    async def test_plan_retrieves_context(self, orchestrator_mocks, sample_task):
        ...
        planning_llm.chat = AsyncMock(return_value=LLMResponse(
    ...
    async def test_agentic_tool_loop(self, orchestrator_mocks, sample_task):
        ...
        resp1 = LLMResponse(
            ...
            tool_calls=[{"id": "call_1", "type": "function", "function": {"name": "search_web", "arguments": '{"query": "foo"}'}}]
        )
        # 2. Final Plan
        resp2 = LLMResponse(
            content="Plan: Use info found.",
            prompt_tokens=20, completion_tokens=10, total_tokens=30, model="test"
        )
        ...
        f1 = asyncio.Future(); f1.set_result(resp1)
        f2 = asyncio.Future(); f2.set_result(resp2)
...
class TestOrchestratorEdgeCases:
    ...
    @pytest.fixture
    def basic_orchestrator(self, mock_gateway, mock_config):
    ...
    def test_detect_test_command_empty_dir(self, basic_orchestrator, tmp_path):
        """Test test command detection on empty directory."""
        result = basic_orchestrator._detect_test_command(str(tmp_path))
        assert result is None

    def test_detect_test_command_npm(self, basic_orchestrator, tmp_path):
    ...
    def test_detect_test_command_composer(self, basic_orchestrator, tmp_path):
    ...
    def test_detect_test_command_pytest(self, basic_orchestrator, tmp_path):
    ...
    def test_detect_test_command_cargo(self, basic_orchestrator, tmp_path):
    ...
    def test_detect_test_command_priority(self, basic_orchestrator, tmp_path):
    ...
    def test_detect_test_command_invalid_package_json(self, basic_orchestrator, tmp_path):
...
class TestCancellationBehavior:
    ...
    @pytest.fixture
    def cancellable_setup(self, mock_gateway, mock_config):
    ...
    def test_queue_tracks_cancellation(self, cancellable_setup):
        """Test that queue correctly reports cancellation status."""
        queue = cancellable_setup['queue']
        assert queue.is_cancel_requested() == True
...
class TestRetryBehavior:
    ...
    def test_max_retries_configuration(self, max_retries, expected_attempts, mock_gateway):
...
class TestPRBodyGeneration:
    ...
    @pytest.fixture
    def orchestrator_for_pr(self, mock_gateway, mock_config, tmp_path):
    ...
    def test_pr_body_with_no_changes(self, orchestrator_for_pr, sample_task):
    ...
    def test_pr_body_with_many_changes(self, orchestrator_for_pr, sample_task):
    ...
    def test_pr_body_with_special_characters(self, orchestrator_for_pr):
